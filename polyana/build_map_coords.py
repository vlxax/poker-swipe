...
def arcgis(q: str):
    params = {
        "SingleLine": q, "f": "json", "maxLocations": "4", "countryCode": "RUS",
        "searchExtent": f"{MIN_LNG},{MIN_LAT},{MAX_LNG},{MAX_LAT}",
        "outFields": "Match_addr,Addr_type,City,District,Region",
    }
    url = "https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates?" + urlencode(params)
    try:
        data = fetch_json(url)
    except (HTTPError, URLError, TimeoutError, json.JSONDecodeError):
        data = {}
    for row in (data.get("candidates") or []) if isinstance(data, dict) else []:
        loc = row.get("location") or {}
        lat, lng = loc.get("y"), loc.get("x")
        if float(row.get("score") or 0) >= 82 and valid(lat, lng):
            return float(lat), float(lng), "arcgis"
    return None
...
def resolve(club):
    # Fast provider first; slower public geocoders are fallbacks. Geocoding
    # happens in GitHub Actions, never on the user's phone.
...
def main():
    ...
    for idx, club in enumerate(clubs, 1):
        ...
        if valid(club.get("lat"), club.get("lng")):
            resolved_rows.append({
                "id": cid, "name": name, "address": address,
                "lat": float(club["lat"]), "lng": float(club["lng"]),
                "status": "provided_source"
            })
            print(f"[{idx}/{total}] provided {name}")
            continue
    ...
    # Never overwrite a better published map with a worse rebuild.
    previous_count = int(existing_payload.get("resolved_count") or 0)
    if previous_count and len(resolved_rows) < previous_count:
        raise SystemExit(f"Map regression blocked: {len(resolved_rows)} < previous {previous_count}")
    minimum = max(45, int(total * 0.90))
    if len(resolved_rows) < minimum:
        raise SystemExit(f"Too few coordinates resolved: {len(resolved_rows)}/{total}; need at least {minimum}")
